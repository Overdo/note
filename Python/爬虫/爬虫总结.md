...
[![img](http://www.pythonclub.org/_media/python-network-application/post2.png)](http://www.pythonclub.org/_detail/python-network-application/post2.png?id=python-network-application%3Aobserver-spider)
...
有时候即使做了3.1-3.4，访问还是会被据，那么没办法，老老实实把httpfox中看到的headers全都写上，那一般也就行了。 再不行，那就只能用终极绝招了，[selenium](http://seleniumhq.org/)直接控制浏览器来进行访问，只要浏览器可以做到的，那么它也可以做到。类似的还有pamie，watir，等等等等。
